...
def preprocessing(original_tweet):
    ...
    #print(result4)
    #Mapping Emoticons to sentiment
    result5=re.sub(r"['^(;\)|:\)|:\D)+$']",'happy',result4)
    result6=re.sub(r"['^(:\()+$']",'sad',result5)
    #smiley_pattern =  # matches only the smileys ":)" and ":("
    ##Remove special characters
    result7=re.sub(r"[!@#$%^&*()_+-=:;?/~`'’]",' ',result6)
    #print("After removing special characters:\n")
    #print(result5)
    ##Tweet tokenizer
    tkznr=TweetTokenizer(reduce_len=True,strip_handles=True,preserve_case=False)
    result8=tkznr.tokenize(result7)
    #print("After Tweet Tokenizing:\n")
    #print(result6)
    result9=" ".join(result8)
    corr_tweet=result9
    #print(corr_tweet)
    return corr_tweet

#Creating Emoticons dictionary
emoticon={':)':'happy'},{':D':'happy'},{';)':'happy'},{':(':'sad'}
        
n_gram=2;
tweets=[]
all_ngrams=[]
pos_tweets=[("@Microsoft 2nd computer with same error!!! #Windows10fail Guess we will shelve this until SP1! http://t.co/QCcHlKuy8Q","positive")]

for cols in actual_training_data:
    ngram_sentence=[]
    preprocessed_sentence=preprocessing(pos_tweets)
